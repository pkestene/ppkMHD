 ...
 */
template <int dim>
class SolverMHDMuscl : public ppkMHD::SolverBase
{
...
template <int dim>
void
SolverMHDMuscl<dim>::init_wave(DataArray Udata)
{

  WaveParams wParams = WaveParams(configMap);

  using InitWaveFunctor =
    typename std::conditional<dim == 2, InitWaveFunctor2D_MHD, InitWaveFunctor3D_MHD>::type;

  // perform init
  InitWaveFunctor::apply(params, wParams, Udata);
