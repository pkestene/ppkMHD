...
public:
  ...
  /**
   * 2D Convert conservative variables (rho, rho*u, rho*v, e) to 
  ...
  /**
   * 3D Convert conservative variables (rho, rho*u, rho*v, rho*w, e) to 
   * primitive variables (rho,u,v,p)
   * @param[in]  u  conservative variables array
   * @param[out] q  primitive    variables array (allocated in calling routine, size is constant nbvar)
   * @param[out] c  local speed of sound
  ...
  template<int dim_ = dim>
  KOKKOS_INLINE_FUNCTION
  void trace_unsplit_2d(const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& q, 
			const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qNeighbors_0,
			const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qNeighbors_1,
			const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qNeighbors_2,
			const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qNeighbors_3,
			real_t c, 
			real_t dtdx, 
			real_t dtdy, 
			typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qm_x,
			typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qm_y,
			typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qp_x,
			typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qp_y) const
  {
  ...
  /**
   * Trace computations for 3d unsplit Godunov scheme.
   *
   * \param[in] q          : Primitive variables state.
   * \param[in] qNeighbors : state in the neighbor cells (2 neighbors
   * per dimension, in the following order x+, x-, y+, y-, z+, z-)
  ...
  template<int dim_ = dim>
  KOKKOS_INLINE_FUNCTION
  void trace_unsplit_3d(const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& q, 
			const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qNeighbors_0,
			const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qNeighbors_1,
			const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qNeighbors_2,
			const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qNeighbors_3,
			const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qNeighbors_4,
			const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qNeighbors_5,
			real_t c, 
			real_t dtdx, 
			real_t dtdy,
			real_t dtdz,
			typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qm_x,
			typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qm_y,
			typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qm_z,
			typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qp_x,
			typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qp_y,
			typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qp_z) const
  {
  ...
  template<int dim_ = dim>
  KOKKOS_INLINE_FUNCTION
  void trace_unsplit_2d_along_dir(const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& q, 
				  const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& dqX,
				  const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& dqY,
				  real_t dtdx, 
				  real_t dtdy, 
				  int    faceId,
				  typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qface) const
  {
  ...
  template<int dim_ = dim>
  KOKKOS_INLINE_FUNCTION
  void trace_unsplit_3d_along_dir(const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& q, 
				  const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& dqX,
				  const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& dqY,
				  const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& dqZ,
				  real_t dtdx, 
				  real_t dtdy, 
				  real_t dtdz, 
				  int    faceId,
				  typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qface) const
  {
  ...
  template<int dim_ = dim>
  KOKKOS_INLINE_FUNCTION
  void trace_unsplit_hydro_2d(const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& q,
			      const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& dqX,
			      const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& dqY,
			      real_t dtdx,
			      real_t dtdy,
  ...
  template<int dim_ = dim>
  KOKKOS_INLINE_FUNCTION
  void slope_unsplit_hydro_2d(const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& q, 
			      const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qPlusX, 
			      const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qMinusX,
			      const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qPlusY,
			      const typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& qMinusY,
			      typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& dqX,
			      typename Kokkos::Impl::enable_if<dim_==2, HydroState>::type& dqY) const
  {
  ...
  /**
   * Compute primitive variables slope (vector dq) from q and its neighbors 3D.
  ...
  template<int dim_ = dim>
  KOKKOS_INLINE_FUNCTION
  void slope_unsplit_hydro_3d(const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& q, 
			      const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qPlusX, 
			      const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qMinusX,
			      const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qPlusY,
			      const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qMinusY,
			      const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qPlusZ,
			      const typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& qMinusZ,
			      typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& dqX,
			      typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& dqY,
			      typename Kokkos::Impl::enable_if<dim_==3, HydroState>::type& dqZ) const
  {
