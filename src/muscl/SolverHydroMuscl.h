...
public:
  ...
  void
  init_rising_bubble(DataArray Udata, VectorField gravity); // 2d and 3d
  void
  init_disk(DataArray Udata, VectorField gravity); // 2d and 3d
...
template <int dim>
void
SolverHydroMuscl<dim>::init_rising_bubble(DataArray Udata, VectorField gravity)
{

  RisingBubbleParams rbParams = RisingBubbleParams(configMap);

  // alias to actual device functor
  using RBFunctor =
    typename std::conditional<dim == 2, RisingBubbleFunctor2D, RisingBubbleFunctor3D>::type;
...
template <int dim>
void
SolverHydroMuscl<dim>::init_disk(DataArray Udata, VectorField gravity)
{
  ...
  using InitDiskFunctor =
    typename std::conditional<dim == 2, InitDiskFunctor2D, InitDiskFunctor3D>::type;
