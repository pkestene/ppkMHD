...
public:
  ...
  InitImplodeFunctor(HydroParams params, MonomMap monomMap, DataArray Udata)
...
public:
  ...
  InitBlastFunctor(HydroParams params, MonomMap monomMap, BlastParams bParams, DataArray Udata)
    : MoodBaseFunctor<dim, degree>(params, monomMap)
    , bParams(bParams)
    , Udata(Udata){};
...
public:
  ...
  operator()(const typename std::enable_if<dim_ == 3, int>::type & index) const
  {
    ...
    else if (khParams.p_sine_rob)
    {
      ...
      Udata(i, j, k, IP) = pressure / (gamma0 - 1.0) + 0.5 *
...
public:
  ...
  InitWedgeFunctor(HydroParams params, MonomMap monomMap, WedgeParams wparams, DataArray Udata)
    : MoodBaseFunctor<dim, degree>(params, monomMap)
    , Udata(Udata)
    , wparams(wparams){};
...
public:
  ...
  real_t
  compute_T(real_t x, real_t y) const
  {
  ...
  KOKKOS_INLINE_FUNCTION
  real_t
  compute_rho(real_t x, real_t y) const
  {
    // compute temperature
  ...
  KOKKOS_INLINE_FUNCTION
  real_t
  compute_u(real_t x, real_t y) const
  {
  ...
  KOKKOS_INLINE_FUNCTION
  real_t
  compute_v(real_t x, real_t y) const
  {
  ...
  KOKKOS_INLINE_FUNCTION
  void
  compute_HydroState_with_quadrature(real_t x, real_t y, int nQuadPts, HydroState2d & q) const
  {
