...
public:
  ...
  MoodBaseFunctor(HydroParams params, typename MonomialMap<dim, degree>::MonomMap monomMap)
  ...
  isValid(const typename std::enable_if<dim_ == 2, HydroState>::type & u) const
  {
  ...
  /**
   * Given an hydrodynamics state in input (rho, rho*u, rho*v, rho*w, e), check if the state is
   * valid.
   *
   * More precisely, just check if density and pressure are positive.
   *
  ...
  template <int dim_ = dim>
  KOKKOS_INLINE_FUNCTION int
  isValid(const typename std::enable_if<dim_ == 3, HydroState>::type & u) const
  {
