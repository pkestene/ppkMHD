...
GlobalMpiSession::GlobalMpiSession(int * argc, char *** argv, std::ostream * out)
{
  ...
  TEST_FOR_EXCEPTION_PRINT(
    mpiHasBeenStarted, std::runtime_error, "Error, you can only call this constructor once!", out);
  ...
  TEST_FOR_EXCEPTION_PRINT(mpierr != 0,
                           std::runtime_error,
  ...
  oss << "hydroSimu::GlobalMpiSession::GlobalMpiSession(): started MPI process on host with name "
      << procName << " and rank " << rank_ << " out of " << nProc_ << " in MPI_COMM_WORLD !"
      << std::endl;

#else

  oss << "hydroSimu::GlobalMpiSession::GlobalMpiSession(): started serial run" << std::endl;
...
// =======================================================
void
GlobalMpiSession::initialize(std::ostream * out)
{
#ifdef USE_MPI
  ...
  if (mpierr != 0)
  {
    *out << "Error code=" << mpierr << " detected in MPI_Comm_rank()" << std::endl;
  }

  mpierr = ::MPI_Comm_size(MPI_COMM_WORLD, &nProc_);
  if (mpierr != 0)
  {
    *out << "Error code=" << mpierr << " detected in MPI_Comm_size()" << std::endl;
  }
