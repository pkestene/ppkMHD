...
template <int N>
void
write_nodes_location(std::ostream &          outFile,
        ...
        for (int idy = 0; idy < N + 1; ++idy)
        {
          for (int idx = 0; idx < N + 1; ++idx)
          {
            ...
            else
            {
              ...
              x = xo +
                  0.5 *
                    (sdm_geom.solution_pts_1d_host(idx - 1) + sdm_geom.solution_pts_1d_host(idx)) *
                    dx;
            }
            ...
            else
            {
              ...
              y = yo +
                  0.5 *
                    (sdm_geom.solution_pts_1d_host(idy - 1) + sdm_geom.solution_pts_1d_host(idy)) *
                    dy;
            }
            ...
            for (int idy = 0; idy < N + 1; ++idy)
            {
              for (int idx = 0; idx < N + 1; ++idx)
              {
                ...
                else
                {
                  ...
                  x = xo + 0.5 *
                             (sdm_geom.solution_pts_1d_host(idx - 1) +
                              sdm_geom.solution_pts_1d_host(idx)) *
                             dx;
                }
                ...
                else
                {
                  ...
                  y = yo + 0.5 *
                             (sdm_geom.solution_pts_1d_host(idy - 1) +
                              sdm_geom.solution_pts_1d_host(idy)) *
                             dy;
                }
                ...
                else
                {
                  ...
                  z = zo + 0.5 *
                             (sdm_geom.solution_pts_1d_host(idz - 1) +
                              sdm_geom.solution_pts_1d_host(idz)) *
                             dz;
                }
...
template <int N>
void
write_cells_connectivity(std::ostream &          outFile,
                         sdm::SDM_Geometry<2, N> sdm_geom,
                         HydroParams &           params,
                         ConfigMap &             configMap,
                         uint64_t &              offsetBytes)
...
template <int N>
void
write_cells_data(std::ostream &                     outFile,
                 ...
                 uint64_t &                         offsetBytes)
{
  ...
  for (int iVar = 0; iVar < nbvar; iVar++)
  {
    ...
    outFile << "    <DataArray type=\"" << dataType << "\" Name=\"" << variables_names.at(iVar)
            ...
            for (int idz = 0; idz < N; ++idz)
            {
              for (int idy = 0; idy < N; ++idy)
              {
                for (int idx = 0; idx < N; ++idx)
                {
                  ...
                  real_t data =
                    Uhost(gw + i, gw + j, gw + k, sdm::DofMap<3, N>(idx, idy, idz, iVar));
                  outFile << data << " ";
...
template <int N>
void
write_appended_binary_data(std::ostream &                     outFile,
                           DataArray2d::HostMirror            Uhost,
                           sdm::SDM_Geometry<2, N>            sdm_geom,
                           HydroParams &                      params,
                           ConfigMap &                        configMap,
...
template <int N>
void
save_VTK_SDM(DataArray2d                        Udata,
             DataArray2d::HostMirror            Uhost,
             HydroParams &                      params,
             ConfigMap &                        configMap,
             sdm::SDM_Geometry<2, N>            sdm_geom,
             ...
             std::string                        debug_name = "")
{
  ...
  if (params.myRank == 0)
  {
    ...
    if (!debug_name.empty())
      headerFilename =
        outputDir + "/" + outputPrefix + "_" + debug_name + "_time" + stepNum.str() + ".pvtu";

    write_pvtu_header(
      headerFilename, outputPrefix, params, configMap, nbvar, variables_names, iStep);
  }
#endif // USE_MPI
...
#ifdef USE_MPI
  {
    ...
    filename =
      outputDir + "/" + outputPrefix + "_time" + stepNum.str() + "_mpi" + rankFormat.str() + ".vtu";
  }
#endif // USE_MPI
  ...
  outFile << "<Piece NumberOfPoints=\"" << nbOfNodes << "\" NumberOfCells=\"" << nbOfCells
          << "\" >\n";
...
template <int N>
void
save_VTK_SDM(DataArray3d                        Udata,
             DataArray3d::HostMirror            Uhost,
             HydroParams &                      params,
             ConfigMap &                        configMap,
             sdm::SDM_Geometry<3, N>            sdm_geom,
