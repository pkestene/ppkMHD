  ...
  if (outputVtkAscii)
  {
    ...
    for (iVar = 0; iVar < nbvar; iVar++)
    {
      ...
      for (int index = 0; index < nbCells; ++index)
      {
        ...
        if (k >= kmin + ghostWidth and k <= kmax - ghostWidth and j >= jmin + ghostWidth and
            j <= jmax - ghostWidth and i >= imin + ghostWidth and i <= imax - ghostWidth)
        {
                ...
                std::string                        debug_name)
{
  ...
  std::string filename;
  if (debug_name.empty())
    filename = outputDir + "/" + outputPrefix + "_time" + timeFormat.str() + "_mpi" +
               rankFormat.str() + ".vti";
  else
    filename = outputDir + "/" + outputPrefix + "_" + debug_name + "_time" + timeFormat.str() +
               "_mpi" + rankFormat.str() + ".vti";

  // header file : parallel vti format
  std::string headerFilename =
    outputDir + "/" + outputPrefix + "_time" + timeFormat.str() + ".pvti";
  ...
  if (params.myRank == 0)
  {
    write_pvti_header(headerFilename, outputPrefix, params, nbvar, variables_names, iStep);
  }
...
// =======================================================
void
write_pvti_header(std::string                        headerFilename,
                  ...
                  int                                iStep)
{
  ...
  {
    if (useDouble)
      outHeader << "      <PDataArray type=\"Float64\" Name=\"" << varNames.at(iVar) << "\"/>"
                << std::endl;
    else
      outHeader << "      <PDataArray type=\"Float32\" Name=\"" << varNames.at(iVar) << "\"/>"
                << std::endl;
  }
