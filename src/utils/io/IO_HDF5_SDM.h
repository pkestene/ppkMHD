#ifndef IO_HDF5_SDM_H_
#define IO_HDF5_SDM_H_
...
inline int get_io_nodes_per_cell(int dim)
{
  return dim == 2 ? 2 : 3;
}

inline std::string get_io_topology_type(int dim)
{
  return dim == 2 ? "Quadrilateral" : "Hexahedron";
}
 ...
 */
void writeXdmfForHdf5Wrapper_SDM(HydroParams& params,
				 ConfigMap& configMap,
...
// =======================================================
/**
 * Here the SDM mesh is treated like an unstructured mesh, 
 * with one cell per DoF.
 *
 * We might think of something more clever later.
 */
template<DimensionType d, int N>
class Save_HDF5_SDM
{
public:
  //! Decide at compile-time which data array type to use
  using DataArray  = typename std::conditional<d==TWO_D,
					       DataArray2d,
					       DataArray3d>::type;
  using DataArrayHost  = typename std::conditional<d==TWO_D,
						   DataArray2dHost,
						   DataArray3dHost>::type;

  Save_HDF5_SDM(DataArray     Udata,
		DataArrayHost Uhost,
		HydroParams& params,
		ConfigMap& configMap,
		int nbvar,
...
//   Save_HDF5_SDM_mpi(DataArray     Udata,
// 		    DataArrayHost Uhost,
// 		    HydroParams& params,
// 		    ConfigMap& configMap,
// 		    int nbvar,
...
//  */
// template<DimensionType d>
// class Load_HDF5_SDM_mpi : public Load_HDF5_SDM<d>
// {
// public:
//   //! Decide at compile-time which data array type to use
//   using DataArray  = typename std::conditional<d==TWO_D,DataArray2d,DataArray3d>::type;
...
//    *
//    */
//   Load_HDF5_SDM_mpi(DataArray     Udata,
// 		HydroParams& params,
// 		ConfigMap& configMap,
// 		int nbvar,
// 		const std::map<int, std::string>& variables_names) :
//     Load_HDF5_SDM<d>(Udata, params, configMap, nbvar, variables_names)
//   {}; // end constructor
...
//   } // Load_HDF5_SDM_mpi::load
  

// }; // class Load_HDF5_SDM_mpi
