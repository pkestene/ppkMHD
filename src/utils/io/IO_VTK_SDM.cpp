...
// =======================================================
// =======================================================
void write_metadata(std::ostream& outFile,
		    int iStep,
		    real_t time)
{
			  ...
			  ConfigMap& configMap)
{
  ...
  for (int j=0; j<ny; ++j) {
    for (int i=0; i<nx; ++i) {
      ...
      // cell offset
      real_t xo = xmin + (i+nx*i_mpi-ghostWidth)*dx;
      real_t yo = ymin + (j+ny*j_mpi-ghostWidth)*dy;
	...
	for (int idx=0; idx<N+1; ++idx) {
	  ...
	  if (idx > 0) {
	    x += 0.5*(sdm_geom.solution_pts_host(idx-1, idy, IX) +
		      sdm_geom.solution_pts_host(idx  , idy, IX) ) * dx;
	  }
	  
	  if (idy > 0) {
	    y += 0.5*(sdm_geom.solution_pts_host(idx, idy-1, IY) +
		      sdm_geom.solution_pts_host(idx, idy  , IY) ) * dy;
	  }
		  ...
		  real_t time)
{
  ...
  // write metadata (time and iStep)
  write_metadata(outFile, iStep, time);
