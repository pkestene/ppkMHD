...
public:
  ...
  apply(HydroParams params, SDM_Geometry<dim, N> sdm_geom, DataArray UdataSol, DataArray UdataFlux)
  {
    ...
    Interpolate_At_FluxPoints_Functor functor(params, sdm_geom, UdataSol, UdataFlux);
...
public:
  ...
  apply(HydroParams params, SDM_Geometry<dim, N> sdm_geom, DataArray UdataFlux, DataArray UdataSol)
  {
    ...
    Interpolate_At_SolutionPoints_Functor functor(params, sdm_geom, UdataFlux, UdataSol);
    ...
    if (dir == IX)
    {
      ...
      for (int idy = 0; idy < N; ++idy)
      {
        ...
        for (int ivar = 0; ivar < nbvar; ++ivar)
        {
          ...
          // interpolate at flux points for this given variable
          if (itype == INTERPOLATE_SOLUTION or itype == INTERPOLATE_SOLUTION_NEGATIVE or
