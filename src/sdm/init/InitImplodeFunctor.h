...
public:
  ...
  static void
  apply(HydroParams params, SDM_Geometry<dim, N> sdm_geom, ImplodeParams iparams, DataArray Udata)
  {
    int nbCells =
      ...
      for (int idx = 0; idx < N; ++idx)
      {
        ...
        if (tmp)
        {
          ...
          Udata(i, j, dofMap(idx, idy, 0, IE)) =
            p_out / (gamma0 - 1.0) + 0.5 * rho_out * (u_out * u_out + v_out * v_out);
        ...
        else
        {
          ...
          Udata(i, j, dofMap(idx, idy, 0, IE)) =
            p_in / (gamma0 - 1.0) + 0.5 * rho_in * (u_in * u_in + v_in * v_in);
