...
public:
  ...
  apply(HydroParams          params,
        ...
        int                  nbCells)
  {
    Interpolate_At_FluxPoints_Functor_v2 functor(params, sdm_geom, UdataSol, UdataFlux);
...
public:
  ...
  apply(HydroParams          params,
        ...
        int                  nbCells)
  {
    Interpolate_At_SolutionPoints_Functor_v2 functor(params, sdm_geom, UdataFlux, UdataSol);
    Kokkos::parallel_for(nbCells, functor);
  }
    ...
    if (dir == IX)
    {
      ...
      for (int idy = 0; idy < N; ++idy)
      {
        ...
        for (int ivar = 0; ivar < nbvar; ++ivar)
        {
          ...
          // interpolate at flux points for this given variable
          if (itype == INTERPOLATE_SOLUTION or itype == INTERPOLATE_SOLUTION_NEGATIVE)
          ...
          for (int idx = 0; idx < N; ++idx)
          {
            ...
            if (itype == INTERPOLATE_DERIVATIVE_NEGATIVE or itype == INTERPOLATE_SOLUTION_NEGATIVE)
