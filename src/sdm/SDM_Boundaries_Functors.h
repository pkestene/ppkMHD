...
public:
  ...
  MakeBoundariesFunctor_SDM(HydroParams params, SDM_Geometry<dim, N> sdm_geom, DataArray Udata)
    : SDMBaseFunctor<dim, N>(params, sdm_geom)
    , Udata(Udata){};

  // static method which does it all: create and execute functor
  static void
  apply(HydroParams params, SDM_Geometry<dim, N> sdm_geom, DataArray Udata, int nbIter)
  {
          ...
          for (int idx = 0; idx < N; ++idx)
          {
            ...
            for (iVar = 0; iVar < nbvar; iVar++)
            {
              ...
              else if (boundary_type == BC_NEUMANN)
              {
                ...
                // the inside
                i0 = ghostWidth;
                Udata(i, j, dofMap(idx, idy, 0, iVar)) = Udata(i0, j, dofMap(idx, idy, 0, iVar));
              }
              else
              { // periodic
          ...
          for (int idx = 0; idx < N; ++idx)
          {
            ...
            for (iVar = 0; iVar < nbvar; iVar++)
            {
              ...
              else if (boundary_type == BC_NEUMANN)
              {
                ...
                j0 = ghostWidth;
                Udata(i, j, dofMap(idx, idy, 0, iVar)) = Udata(i, j0, dofMap(idx, idy, 0, iVar));
              }
              else
              { // periodic
  ...
  operator()(const typename std::enable_if<dim_ == 3, int>::type & index) const
  {
    ...
    if (faceId == FACE_XMIN)
    {
      ...
      if (k >= kmin && k <= kmax && j >= jmin && j <= jmax && i >= 0 && i < ghostWidth)
      {
    ...
    if (faceId == FACE_XMAX)
    {
      ...
      if (k >= kmin && k <= kmax && j >= jmin && j <= jmax && i >= nx + ghostWidth &&
          i <= nx + 2 * ghostWidth - 1)
      {
    ...
    if (faceId == FACE_YMIN)
    {
      ...
      if (k >= kmin && k <= kmax && j >= 0 && j < ghostWidth && i >= imin && i <= imax)
      {
    ...
    if (faceId == FACE_YMAX)
    {
      ...
      if (k >= kmin && k <= kmax && j >= ny + ghostWidth && j <= ny + 2 * ghostWidth - 1 &&
          i >= imin && i <= imax)
      {
    ...
    if (faceId == FACE_ZMIN)
    {
      ...
      if (k >= 0 && k < ghostWidth && j >= jmin && j <= jmax && i >= imin && i <= imax)
      {
    ...
    if (faceId == FACE_ZMAX)
    {
      ...
      if (k >= nz + ghostWidth && k <= nz + 2 * ghostWidth - 1 && j >= jmin && j <= jmax &&
          i >= imin && i <= imax)
      {
