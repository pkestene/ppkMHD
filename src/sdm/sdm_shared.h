...
DofMap(int i, int j, int k, int iv)
{
...
DofMapFlux(int i, int j, int k, int iv)
{

  if (dir == IX)
  {

    return dim == 2 ? i + (N + 1) * j + (N + 1) * N * iv
                    : i + (N + 1) * j + (N + 1) * N * k + (N + 1) * N * N * iv;
  }
  else if (dir == IY)
  {

    return dim == 2 ? i + N * j + N * (N + 1) * iv
                    : i + N * j + N * (N + 1) * k + N * (N + 1) * N * iv;
  }
  else if (dir == IZ)
  {

    return dim == 2 ? 0 : i + N * j + N * N * k + N * N * (N + 1) * iv;
  }
  else
  {
