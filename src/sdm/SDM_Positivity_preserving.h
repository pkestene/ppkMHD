...
public:
        ...
        DataArray            Uaverage)
  {
    ...
    Apply_positivity_Functor functor(params, sdm_geom, UdataSol, UdataFlux, Uaverage);
  ...
  operator()(const typename std::enable_if<dim_ == 2, int>::type & index) const
  {
    ...
    if (theta1 < 1.0)
    {
      ...
      { // dir == IY, we need to swap idx <-> idy
    ...
    for (int idy = 0; idy < idy_flux_end; ++idy)
    {
      for (int idx = 0; idx < idx_flux_end; ++idx)
      {
        ...
        if (pressure < 1e-12)
        {
          ...
          real_t b1 = 2.0 * drho * (e_ave - eps2 / (gamma0 - 1.0)) + 2.0 * rho_ave * dE -
                      2.0 * (rhou_ave * drhou + rhov_ave * drhov);
          real_t c1 = 2.0 * rho_ave * e_ave - (rhou_ave * rhou_ave + rhov_ave * rhov_ave) -
      ...
      for (int idy = 0; idy < idy_flux_end; ++idy)
      {
        for (int idx = 0; idx < idx_flux_end; ++idx)
        {
          ...
          const real_t pressure =
            (gamma0 - 1) * (E - 0.5 * (rhou * rhou + rhov * rhov + rhow * rhow) / rho);
          ...
          if (pressure < 1e-12)
          {
            ...
            real_t dm2 = drhou * drhou + drhov * drhov + drhow * drhow;
...
public:
  ...
  // static method which does it all: create and execute functor
  static void
  apply(HydroParams params, SDM_Geometry<dim, N> sdm_geom, DataArray UdataSol, DataArray Uaverage)
  {
    int64_t nbCells =
      dim == 2 ? params.isize * params.jsize : params.isize * params.jsize * params.ksize;

    Apply_positivity_Functor_v2 functor(params, sdm_geom, UdataSol, Uaverage);
