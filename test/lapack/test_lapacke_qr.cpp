...
void
print_matrix_rowmajor(char * desc, lapack_int m, lapack_int n, double * mat, lapack_int ldm)
{
  lapack_int i, j;
  printf("\n %s\n", desc);
...
void
print_vector(char * desc, lapack_int n, lapack_int * vec)
{
  lapack_int j;
  printf("\n %s\n", desc);
  for (j = 0; j < n; j++)
...
main(int argc, char * argv[])
{
  ...
  cblas_dtrsm(
    CblasRowMajor, CblasLeft, CblasUpper, CblasTrans, CblasNonUnit, n, m, 1.0, A, lda, AT, ldat);
  ...
  cblas_dtrsm(
    CblasRowMajor, CblasLeft, CblasUpper, CblasNoTrans, CblasNonUnit, n, m, 1.0, A, lda, AT, ldat);
