 ...
 * \f$ \int_{0}^{1} f(x) dx \simeq \sum_{i=1}^N \omega_i f(x_i) \sqrt{x_i(1-x_i)}\f$ where now the quadrature points are \f$ x_i = 0.5 \( 1 - \cos (\frac{2i-1}{2N}\pi) \) \f$ as in the SDM solution points.
 *
 * See also:
...
real_t f_0(real_t x) { return 2.23; }
real_t f_1(real_t x) { return 2 * x + 1; }
real_t f_2(real_t x) { return x * x + 1; }
real_t f_3(real_t x) { return - x*x*x + 4*x*x + x - 1; }
real_t f_4(real_t x) { return  2*x*x*x*x - 4*x*x + 7*x + 3; }
real_t f_5(real_t x) { return 2.5*x*x*x*x*x - 16*x*x + x + 1; }
real_t f_6(real_t x) { return 5*x*x*x*x*x*x - 16*x*x*x - x - 5; }
real_t f_7(real_t x) { return cos(x)+0.23*2*x; }
real_t f_8(real_t x) { return 1.0/(x+1); }

// the corresponding primitives
real_t pf_0(real_t x) { return 2.23*x; }
real_t pf_1(real_t x) { return x*x + x; }
real_t pf_2(real_t x) { return x*x*x/3 + x; }
real_t pf_3(real_t x) { return - x*x*x*x/4 + 4*x*x*x/3 + x*x/2 - x; }
real_t pf_4(real_t x) { return  2*x*x*x*x*x/5 - 4*x*x*x/3 + 7*x*x/2 + 3*x; }
real_t pf_5(real_t x) { return 2.5*x*x*x*x*x*x/6 - 16*x*x*x/3 + x*x/2 + 1; }
real_t pf_6(real_t x) { return 5*x*x*x*x*x*x*x/7 - 16*x*x*x*x/4 - x*x/2 - 5*x; }
real_t pf_7(real_t x) { return sin(x)+0.23*x*x; }
real_t pf_8(real_t x) { return log(x+1); }
...
template<int dim,
	 int N>
void test_chebyshev_quadrature(int i)
{
