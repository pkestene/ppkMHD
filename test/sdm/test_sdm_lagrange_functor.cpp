...
test_lagrange_functor()
{
  ...
  DataArray U = dim == 2 ? DataArray("U", isize, jsize, N * N * params.nbvar)
                         : DataArray("U", isize, jsize, ksize, N * N * N * params.nbvar);

  DataArrayHost UHost = Kokkos::create_mirror(U);

  DataArray U2 = dim == 2 ? DataArray("U2", isize, jsize, N * N * params.nbvar)
  ...
  // init data
  {
    ...
    sdm::InitTestFunctor<dim, N, TEST_DATA_VALUE, 0>::apply(params, sdm_geom, U);

    Kokkos::deep_copy(U2, U);

    // save initial condition
    io_writer->save_data_impl(U, UHost, 0, 0.0, "");
  }

  // call the interpolation functors
  {

    sdm::Interpolate_At_FluxPoints_Functor<dim, N, IY> functor(params, sdm_geom, U, Fluxes);
    Kokkos::parallel_for(nbCells, functor);

    io_writer->template save_flux<IY>(Fluxes, FluxHost, 0, 0.0);
  }
...

  {
    ...
    sdm::Interpolate_At_SolutionPoints_Functor<dim, N, IY, interp> functor(
      params, sdm_geom, Fluxes, U);
    Kokkos::parallel_for(nbCells, functor);
  }
  ...
  if (dim == 2)
  {
    ...
    {
      real_t error_L1 =
        sdm::Compute_Error_Functor_2d<N, sdm::NORM_L1>::apply(params, sdm_geom, U, U2, ivar);
  ...
  else if (dim == 3)
  {
    ...
    {
      real_t error_L1 =
        sdm::Compute_Error_Functor_3d<N, sdm::NORM_L1>::apply(params, sdm_geom, U, U2, ivar);
      printf("L1 error for variable %d : %e\n", ivar, error_L1);
      error_accum += error_L1;
    }
  }
  ...
  // degree less than N, or just small
  {
    ...
    sdm::InitTestFunctor<dim, N, TEST_DATA_VALUE, 1>::apply(params, sdm_geom, U);

    io_writer->save_data_impl(U, UHost, 1, 1.0, "");
  }
...
main(int argc, char * argv[])
{
  ...
  {
    // 2d
    passed = passed and ppkMHD::test_lagrange_functor<2, 4>();

    // 3d
    passed = passed and ppkMHD::test_lagrange_functor<3, 4>();
  }
