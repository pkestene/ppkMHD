...
main(int argc, char * argv[])
{
  ...
  bool periods[NDIM] = { MPI_CART_PERIODIC_TRUE, MPI_CART_PERIODIC_TRUE, MPI_CART_PERIODIC_TRUE };
  ...
  if (numTasks == SIZE_3D)
  {
    ...
    MPI::Cartcomm cartcomm = MPI::COMM_WORLD.Create_cart(NDIM, dims, periods, reorder);
    ...
    cartcomm.Shift((int)MPI_TOPO_DIR_Z, (int)MPI_SHIFT_FORWARD, nbrs[Z_MINUS_1], nbrs[Z_PLUS_1]);
    ...
    for (i = 0; i < N_NEIGHBORS_3D; i++)
    {
      ...
      reqs[i + N_NEIGHBORS_3D] = cartcomm.Irecv(&inbuf[i], 1, MPI::INT, source, tag);
    }
    MPI::Request::Waitall(2 * N_NEIGHBORS_3D, reqs, stats);
    ...
    if (myRank == 0)
    {
      printf("Print topology (COLUMN MAJOR-ORDER) for %dx%dx%d 3D grid:\n", SIZE_X, SIZE_Y, SIZE_Z);
      printf(" rank     i     j     k\n");
    }
  ...
  {
    std::cout << "Must specify " << SIZE_3D << " processors. Terminating.\n";
  }
