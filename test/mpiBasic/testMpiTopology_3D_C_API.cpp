...
main(int argc, char * argv[])
{
  ...
  if (numtasks == SIZE_3D)
  {
    ...
    for (i = 0; i < N_NEIGHBORS_3D; i++)
    {
      ...
      MPI_Irecv(&inbuf[i], 1, MPI_INT, source, tag, MPI_COMM_WORLD, &reqs[i + N_NEIGHBORS_3D]);
    }
    ...
    {
      printf(
        "Print topology (COLUMN MAJOR-ORDER) for a %dx%dx%d 3D grid:\n", SIZE_X, SIZE_Y, SIZE_Z);
      printf(" rank     i     j     k  rank(i+1)  rank(i-1)  rank(j+1)  rank(j-1)  rank(k+1)  "
             "rank(k-1)\n");
    }
