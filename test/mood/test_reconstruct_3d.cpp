 ...
 */
real_t test_function_3d(real_t x, real_t y, real_t z)
{

  return x*x+2.2*x*y+4.1*y*y-5.0+x-3.4*z;
...
public:
  ...
  real_t test_function_3d_(real_t x, real_t y, real_t z) const
  {
  ...
  void operator()(const int& i) const
  {
    ...
    for (int ii=0; ii<stencil_size; ++ii) {
      ...
      int z = stencil.offsets(ii,2);
      
      if (x != 0 or y != 0 or z !=0) { // avoid stencil center
    	rhs[s] = this->test_function_3d_(x,y,z) - this->test_function_3d_(0,0,0);
#ifndef CUDA
    	printf("[DEVICE] [% d,% d,% d,% d] % 7.5f\n",x,y,z,s, rhs[s] + this->test_function_3d_(0,0,0));
#endif
    	s++;
      }
    ...
    coefs_t coefs;
    coefs[0] = this->test_function_3d_(0,0,0);
    ...
    for (int ii=0; ii<stencil_size; ++ii) {
      ...
      eval_point[2] = stencil.offsets(ii,2);
...
int main(int argc, char* argv[])
{
  ...
  for (int ii = 0; ii<stencil_size; ++ii) { // loop over stencil point
    ...
    int z = stencil.offsets_h(ii,2);

    if (x != 0 or y != 0 or z != 0) { // avoid stencil center
   
      printf("stencil point coordinates : % d,% d,% d\n",x,y,z);
	...
	geomMatrix(i,j) = geomTerms.eval_hat(x,y,z,n,m,l);
      }
  ...
  std::cout << "test function computed on host\n";
  {
    ...
    for (int index=0; index < stencil_size; ++index) {
      ...
      int z = stencil.offsets_h(index,2);

      if (x != 0 or y !=0 or z !=0) {
	printf("[% d,% d,% d]  % 7.5f\n",x,y,z,
	       test_function_3d(x,y,z) - test_function_3d(0,0,0));

	rhs_host[rhs_index] = test_function_3d(x,y,z) - test_function_3d(0,0,0);
	rhs_index++;
      }
    }
  }
  ...
  coef_host[0] = test_function_3d(0,0,0);
  ...
  for (int ii=0; ii<stencil_size; ++ii) {
    ...
    eval_point[2] = stencil.offsets_h(ii,2);

    int x = stencil.offsets_h(ii,0);
    int y = stencil.offsets_h(ii,1);
    int z = stencil.offsets_h(ii,2);
    printf("polynomial eval on host [% d, % d,% d]  % 7.5f == % 7.5f\n",x,y,z,
	   polynomial_eval<ncoefs>(eval_point[0], eval_point[1], eval_point[2],
				   monomialMap.data_h, coefs),
	   test_function_3d(x, y, z));
  }
  ...
  std::cout << "test function computed on device\n";
  {
    ...
    for (int index=0; index < stencil_size; ++index) {
      ...
      if (x != 0 or y != 0 or z != 0) { // avoid stencil center
	printf("[% d,% d,% d]  % 7.5f  == % 7.5f\n",x,y,z,result_h(index),test_function_3d(x, y, z));
      }
    }
  }
