...
public:
  ...
  operator()(const typename std::enable_if<dim_ == 2, int>::type & index) const
  {
        ...
        i < isize - ghostWidth + 1)
    {
      ...
      {
        real_t tmp = 0;
        for (int k = 0; k < mat_pi.extent(1); ++k)
        {
...
main(int argc, char * argv[])
{
  ...
  // 2D test
  {
    ...
    // create functor
    mood::TestFunctor<dim, degree, stencilId> f(
      params, monomialMap.data, U, Fluxes_x, Fluxes_y, Fluxes_z, stencil, geomMatrixPI_view);
